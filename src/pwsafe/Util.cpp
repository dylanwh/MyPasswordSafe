...
char GetRandAlphaNumChar()
{
  ...
  return pool[rand() % (26*2+10)];
}

string GetAlphaNumPassword(unsigned int len)
{
  string ret;
  for(unsigned int i = 0; i < len; i++) {
    ret += GetRandAlphaNumChar();
  }
  return ret;
}
